...
def palabras_validas(renglon, LONG_PALABRA_MIN):
    ...
    si no hay q tomar solo las palabras q len(palabra) >= LONG_PALABRA_MIN:

    borrar: 
            Sacar LONG_PALABRA_MIN como parametro ingresado
            if len(palabra_sin) >= LONG_PALABRA_MIN:
                lista_palabra_valida.append(sacar_tildes(palabra_sin))

    y poner como descripción:
        Esta función toma el renglon ingresado por parámetro, lo divide por palabra creando una lista,
        y se fija que las palabras en esta lista no contengan ningun caracter especial, y en caso de tenerlo
        lo "elimina". Y por último devuelve la lista con las palabras ya verificadas.
...
def sacar_tildes(palabra):
    ...
    AGREGUÉ LO DE LA DIERESIS EN LA U...
